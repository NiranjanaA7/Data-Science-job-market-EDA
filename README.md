...
> **Before you run it:** `app.py` currently points to `featured_jobs1.csv`, but the feature engineering notebook saves `featured_jobs.csv`. Update `DATA_PATH` in `app.py` (or rename the CSV) so the filenames match — otherwise the app raises a `FileNotFoundError` on load.
...
- Fix the `featured_jobs` / `featured_jobs1` filename mismatch and switch to relative, config-driven paths
